...
export default async function ChantiersPage() {
  ...
  const { data: chantiers } = await supabase
    .from("chantiers")
    .select("id,nom,reference,statut,type,adresse,ville,prix_ht,client:clients!client_id(nom)")
    ...
    <div>
      <div className="mb-8 flex items-start justify-between">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-bold text-slate-900" style={{ fontFamily:"Georgia,serif" }}>Chantiers</h1>
        </div>
        {isAdmin && (
          <Link href="/chantiers/nouveau" className="flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-bold" style={{ background:"#1A2B5F", color:"#F5C200" }}>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {(chantiers ?? []).map((c: any) => {
          const sc = STATUT[c.statut] ?? STATUT.planifié;
          return (
            <Link key={c.id} href={`/chantiers/${c.id}`}
              ...
              <div className="px-5 py-4" style={{ background:"linear-gradient(135deg,#1A2B5F,#243570)" }}>
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <p className="mb-0.5 text-[10px] font-black tracking-widest" style={{ color:"#F5C200" }}>{c.reference}</p>
                    <p className="truncate font-bold text-white">{c.nom}</p>
                    <p className="text-xs text-white/50">{c.client?.nom}</p>
                  </div>
                  <span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-bold ${sc.bg} ${sc.text}`}>{sc.label}</span>
                </div>
              </div>
              <div className="p-4 space-y-1.5">
                <span className="inline-block rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-600">{c.type}</span>
                {(c.adresse || c.ville) && (
                  <div className="flex items-center gap-1.5 text-xs text-slate-500">
                    <MapPin className="h-3.5 w-3.5 text-slate-300" />
                    <span className="truncate">{[c.adresse, c.ville].filter(Boolean).join(", ")}</span>
                  </div>
                )}
                {c.prix_ht && (
                  <p className="text-right text-sm font-black text-[#1A2B5F]">{fmtEurSans(c.prix_ht)}</p>
                )}
              </div>
            </Link>
          );
