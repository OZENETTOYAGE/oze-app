...
// onSbReady = appel immédiat car sb est dispo synchroniquement
const onSbReady = (fn) => fn(sb);

/* ═══════════════════════════════════════════════════════════════════
   OZÉ NETTOYAGE — APPLICATION COMPLÈTE v4
...
const mapUser = r => ({ id: r.id, nom: r.nom, role: r.role, pin: r.pin, avatar: r.avatar || r.nom?.split(" ").map(n=>n[0]).join("").slice(0,2).toUpperCase(), email: r.email, tel: r.tel, poste: r.poste, actif: r.actif !== false });
const unmapUser = u => ({ id: u.id, nom: u.nom, role: u.role, pin: u.pin, avatar: u.avatar, email: u.email, tel: u.tel, poste: u.poste, actif: u.actif });

const mapChantier = r => ({ id: r.id, nom: r.nom, clientId: r.client_id, adresse: r.adresse, type: r.type, responsableId: r.responsable_id, actif: r.actif !== false });
const unmapChantier = c => ({ id: c.id, nom: c.nom, client_id: c.clientId, adresse: c.adresse, type: c.type, responsable_id: c.responsableId, actif: c.actif });

const mapPointage = r => ({ id: r.id, chantierId: r.chantier_id, nomChantier: r.nom_chantier, agentId: r.agent_id, nomAgent: r.nom_agent, entree: r.entree, sortie: r.sortie });
const unmapPointage = p => ({ id: p.id, chantier_id: p.chantierId, nom_chantier: p.nomChantier, agent_id: p.agentId, nom_agent: p.nomAgent, entree: p.entree, sortie: p.sortie });

const mapDevis = r => ({ id: r.id, clientId: r.client_id, chantierId: r.chantier_id, date: r.date, validite: r.validite, statut: r.statut, lignes: r.lignes || [], notes: r.notes || "", redacteurId: r.redacteur_id });
const unmapDevis = d => ({ id: d.id, client_id: d.clientId, chantier_id: d.chantierId, date: d.date, validite: d.validite, statut: d.statut, lignes: d.lignes, notes: d.notes, redacteur_id: d.redacteurId });
            ...
            <Card key={c.id}>
              ...
              <div style={{ padding: "14px 16px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 7 }}>
                  ...
                  {canEdit && <Btn onClick={() => { const u = chantiers.filter(x => x.id !== c.id); setChantiers(u); }} size="sm" v="danger"><Ico n="trash" s={13} /></Btn>}
                </div>
              </div>
            </Card>
          );
...
// ═══════════════════════════════════════════════════════════════════
function PagePointagesAdmin({ user, pointages, setPointages, users, chantiers, syncStatus }) {
  ...
  const del = async id => {
    const u = pointages.filter(p => p.id !== id); setPointages(u);
    if (syncStatus !== "offline") await sb.from("oze_pointages").delete().eq("id", id);
    else LS.set("oze_pointages", u);
  };
          ...
          return (
            <Card key={d.id} style={{ padding: "16px 20px" }}>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
                ...
                {user.role === "administrateur" && <Btn onClick={() => { const u = devis.filter(x => x.id !== d.id); setDevis(u); }} v="danger" size="sm"><Ico n="trash" s={13} /></Btn>}
              </div>
            </Card>
          );
        })}
...
// PAGE UTILISATEURS
// ═══════════════════════════════════════════════════════════════════
function PageUtilisateurs({ users, setUsers }) {
  const [showForm, setShowForm] = useState(false);
  ...
  const save = () => {
    let u;
    if (editU) u = users.map(x => x.id === editU.id ? { ...x, ...form } : x);
    else u = [{ ...form, id: "U" + uid(), avatar: form.nom.split(" ").map(n => n[0]).join("").slice(0, 2).toUpperCase() }, ...users];
...
export default function App() {
  ...
  useEffect(() => {
    const loadAll = async (client) => {
      try {
        const [u, c, p, d] = await Promise.all([
          client.from("oze_users").select("*"),
          client.from("oze_chantiers").select("*"),
          client.from("oze_pointages").select("*").order("entree", { ascending: false }),
          client.from("oze_devis").select("*").order("date", { ascending: false }),
        ]);
        if (u.error || c.error) throw new Error(u.error?.message || c.error?.message);
        if (u.data?.length) setUsers(u.data.map(mapUser));
        if (c.data?.length) setChantiers(c.data.map(mapChantier));
        if (p.data) setPointages(p.data.map(mapPointage));
        if (d.data?.length) setDevis(d.data.map(mapDevis));
        setSyncStatus("online");
      } catch (e) {
        console.warn("Supabase offline, mode local:", e);
        setUsers(LS.get("oze_users", USERS_INIT));
        setChantiers(LS.get("oze_chantiers", CHANTIERS_INIT));
        setPointages(LS.get("oze_pointages", []));
        setDevis(LS.get("oze_devis", DEVIS_INIT));
        setSyncStatus("offline");
      }
    };
    // Attendre que Supabase soit prêt
    onSbReady(client => loadAll(client));
    // Timeout fallback si Supabase ne charge pas
    const t = setTimeout(() => {
  ...
  useEffect(() => {
    if (syncStatus !== "online" || !sb) return;
    const ch = sb.channel("oze-realtime")
      .on("postgres_changes", { event: "*", schema: "public", table: "oze_pointages" }, ({ eventType, new: n, old: o }) => {
        setPointages(prev => {
          if (eventType === "INSERT") return [mapPointage(n), ...prev.filter(x => x.id !== n.id)];
          if (eventType === "UPDATE") return prev.map(x => x.id === n.id ? mapPointage(n) : x);
          if (eventType === "DELETE") return prev.filter(x => x.id !== o.id);
          return prev;
        });
      })
      .on("postgres_changes", { event: "*", schema: "public", table: "oze_chantiers" }, ({ eventType, new: n, old: o }) => {
        setChantiers(prev => {
          if (eventType === "INSERT") return [mapChantier(n), ...prev.filter(x => x.id !== n.id)];
          if (eventType === "UPDATE") return prev.map(x => x.id === n.id ? mapChantier(n) : x);
          if (eventType === "DELETE") return prev.filter(x => x.id !== o.id);
          return prev;
        });
      })
      .on("postgres_changes", { event: "*", schema: "public", table: "oze_devis" }, ({ eventType, new: n, old: o }) => {
        setDevis(prev => {
          if (eventType === "INSERT") return [mapDevis(n), ...prev.filter(x => x.id !== n.id)];
          if (eventType === "UPDATE") return prev.map(x => x.id === n.id ? mapDevis(n) : x);
          if (eventType === "DELETE") return prev.filter(x => x.id !== o.id);
          return prev;
        });
      })
      .on("postgres_changes", { event: "*", schema: "public", table: "oze_users" }, ({ eventType, new: n }) => {
        if (eventType === "INSERT" || eventType === "UPDATE") setUsers(prev => prev.map(x => x.id === n.id ? mapUser(n) : x).concat(prev.find(x => x.id === n.id) ? [] : [mapUser(n)]));
      })
      .subscribe(status => setSyncStatus(status === "SUBSCRIBED" ? "online" : "offline"));
    return () => sb.removeChannel(ch);
  }, [syncStatus]);

  // ── SETTERS SYNCHRONISÉS ────────────────────────────────────────
  const syncChantiers = async (updated) => {
    setChantiers(updated); LS.set("oze_chantiers", updated);
    if (syncStatus === "online" && sb) {
      const last = updated[0];
      if (last) await sb.from("oze_chantiers").upsert(unmapChantier(last));
    }
  };
  const syncPointages = async (updated) => {
    setPointages(updated); LS.set("oze_pointages", updated);
    if (syncStatus === "online" && sb) {
      const last = updated[0];
      if (last) await sb.from("oze_pointages").upsert(unmapPointage(last));
    }
  };
  const syncDevis = async (updated) => {
    setDevis(updated); LS.set("oze_devis", updated);
    if (syncStatus === "online" && sb) {
      const last = updated[0];
      if (last) await sb.from("oze_devis").upsert(unmapDevis(last));
    }
  };
  const syncUsers = async (updated) => {
    setUsers(updated); LS.set("oze_users", updated);
    if (syncStatus === "online" && sb) {
      const last = updated.find(u => !users.find(x => x.id === u.id)) || updated[0];
      if (last) await sb.from("oze_users").upsert(unmapUser(last));
    }
  };
  ...
  const renderPage = () => {
    const sharedProps = { user, chantiers, setChantiers: syncChantiers, clients: CLIENTS_INIT, users, setUsers: syncUsers, pointages, setPointages: syncPointages, devis, setDevis: syncDevis, push, syncStatus };
    switch (page) {
    ...
    <div style={{ display: "flex", height: "100vh", background: "#f1f4f9", fontFamily: "'DM Sans',sans-serif", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ height: 58, background: "#fff", borderBottom: "1.5px solid #f1f5f9", display: "flex", alignItems: "center", padding: "0 24px", gap: 12, flexShrink: 0 }}>
          ...
          <div style={{ flex: 1 }} />
          {/* Sync indicator */}
          <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, fontWeight: 700, color: syncStatus === "online" ? "#10b981" : syncStatus === "connecting" ? "#f59e0b" : "#9ca3af", background: syncStatus === "online" ? "#dcfce7" : syncStatus === "connecting" ? "#fef9c3" : "#f1f5f9", padding: "4px 10px", borderRadius: 20 }}>
            <div style={{ width: 7, height: 7, borderRadius: "50%", background: syncStatus === "online" ? "#10b981" : syncStatus === "connecting" ? "#f59e0b" : "#9ca3af", animation: syncStatus === "connecting" ? "pulse 1s infinite" : "none" }} />
            {syncStatus === "online" ? "Synchronisé" : syncStatus === "connecting" ? "Connexion…" : "Hors ligne"}
          </div>
